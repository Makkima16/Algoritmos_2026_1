...
Formato del CSV para modo bloque:
  ...
  - El resultado se guarda automáticamente en KGeoMIP/results/block/ como .xlsx.
"""
...
# repositorio (compartida con KQNodes); los resultados sí van a KGeoMIP/results.
...
def _seleccionar_tpm() -> tuple:
    ...
    while True:
        ...
        if tipo == "1":
            directorio_tpm = DATA_ROOT / "samples_binary"
            break
        elif tipo == "2":
            directorio_tpm = DATA_ROOT / "samples_no_binary"
            break
        else:
...
def _guardar_resultado_single(
    ...
    """Guarda el resultado de una ejecución manual en KGeoMIP/results/manually/."""
