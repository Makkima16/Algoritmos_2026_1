...
class SystemCreator:
    ...
    def save_to_csv(self, filename: str = None):
        # Carpeta data/ de la raíz (este script vive dentro de data/).
        data_root = os.path.dirname(os.path.abspath(__file__))
        if self.deterministic:
            target_dir = os.path.join(data_root, 'samples_binary')
        else:
            target_dir = os.path.join(data_root, 'samples_no_binary')
        os.makedirs(target_dir, exist_ok=True)

        if filename is None:
            # La letra codifica el tipo de TPM: 'A' determinística, 'B' no determinística.
            # El primer archivo de ese N/letra va sin número (N10A); si ya existe, el
            # siguiente agrega el número de repetición (N10A2, N10A3, ...).
            letter = 'A' if self.deterministic else 'B'
            base = f'N{self.N}{letter}'
            filename = f'{base}.csv'
            filepath = os.path.join(target_dir, filename)
            repeticion = 2
            while os.path.exists(filepath):
                filename = f'{base}{repeticion}.csv'
                filepath = os.path.join(target_dir, filename)
                repeticion += 1
        else:
            filepath = os.path.join(target_dir, filename)
