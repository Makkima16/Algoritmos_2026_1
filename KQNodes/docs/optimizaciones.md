...
> **Arranque del motor en runs por bloque.** Como el caché se limpia por corrida y la
> PRIMERA corrida de un worker paga costos de una sola vez (primer toque de numpy,
> construcción de tablas, JIT, carga/caché de la TPM), ese arranque NO debe contaminar
> el tiempo de la primera prueba. El dashboard (`dashboard/`) ejecuta en `/api/block` una
> corrida de calentamiento **descartable** antes del lote, con los parámetros de la primera
> prueba válida, y la registra **aparte** como "arranque del motor"; así cada prueba
> guarda solo su tiempo de búsqueda. En la comparación por bloque del dashboard, un empate
> de Φ entre KQNodes y KGeoMIP se marca como **"Ambos"**.
